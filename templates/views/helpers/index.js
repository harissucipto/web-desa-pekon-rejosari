...
var linkNavTemplate = _.template('<a  class="page-link" href="<%= url %>"><%= text %></a>');
	...
	* */
	...
	_helpers.paginationNavigation = function (pages, currentPage, totalPages, options) {
		...
		_.each(pages, function (page, ctr) {
			...
			// wrapup the html
			html += '<li' + liClass + '>' + linkNavTemplate({ url: pageUrl, text: pageText }) + '</li>\n';
		});
		return html;
	};
	...
	_helpers.isLinkBlog = function (slug) {
		return slug === 'kabar' ? '/blog' : '/halaman/'+  slug;
	};

	_helpers.isHaveImage = function (img) {
		return img  ? img : 'http://placehold.it/300x200';
	};

	_helpers.isBlogImage = function (img) {
		return img  ? img : 'http://placehold.it/900x300';
	};

	_helpers.imageBg = function (img) {
		return img  ? img : 'http://placehold.it/1920x400';
	};
